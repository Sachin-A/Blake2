...
void InitBlockValue(block* b, uint8_t int) {
...
/* Memory allocators */
int AllocateMemory(block **memory, uint32_t m_const) {
    if (memory != NULL) {
        *memory = (block*)malloc(sizeof(block)*m_const);
        if (!*memory) {
...
const uint32_t ARGON2_QWORDS_IN_BLOCK = 64;

/* Number of pseudo-random values generated by one call to Blake in Agon2i to generate reference block positions */
const uint32_t ARGON2_ADRESSES_IN_BLOCK = 128;

/* Pre-hashing digest length and it's extension */
...
int Argon2Core(Argon2_Context* context, Argon2_type type) {
    ...
    /* Algin the memory size */
    /* Min blocks is 8*number of lanes */
    ...
    Argon2_instance_t instance = { NULL, context->t_cost, memory_blocks, segment_length, segment_length * ARGON2_SYNC_POINTS, context->lanes, context->threads, type, NULL, print_internals };

    /* Initialize: hash inputs, allocate memory, and fill blocks */
